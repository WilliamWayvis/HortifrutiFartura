...
const DisplayCarnes = () => {
  ...
  const colCount = Math.min(Math.max(carnesQueue.length, 1), 6);
    ...
    >
      ...
      <style>{`
        ...
        @keyframes pulse-card {
          ...
          50% { transform: scale(1.02); box-shadow: 0 0 30px 10px rgba(239,68,68,0.3); }
        }
      ...
      {/* Header */}
      <div className="flex-shrink-0 flex items-center justify-between px-[3vw] py-[1.2vh] bg-red-600">
        <h1 className="text-[3vw] font-black text-white tracking-wider">🥩 AÇOUGUE</h1>
        <div className="text-right">
          <p className="text-[0.9vw] font-semibold text-red-100 uppercase tracking-widest leading-none">Tempo médio de espera</p>
          <p className="text-[2.2vw] font-black text-white leading-tight">{getAverageWaitTime('carnes') ?? '0 min'}</p>
        </div>
      </div>

      {/* TOP HALF — Próximas Senhas */}
      <div className="flex-1 min-h-0 flex flex-col px-[2vw] pt-[1.5vh] pb-[1vh] border-b border-gray-700">
        <p className="text-[1.4vw] font-bold text-red-400 uppercase tracking-widest text-center mb-[1.5vh]">
          🔜 Próximas Senhas
        </p>
        <div className="flex-1 min-h-0 overflow-hidden">
          {carnesQueue.length === 0 ? (
            ...
            >
              {carnesQueue.slice(0, 12).map((item, idx) => (
                <div
                  key={item.id}
                ...
                >
                  ...
                  <span className={`text-[2.8vw] font-black leading-none ${
                    item.priority ? 'text-blue-200' : 'text-red-200'
                  }`}>
                    {item.code}
                  </span>
                </div>
        ...
        <div className="flex-1 min-h-0 flex gap-[1.5vw] overflow-hidden">
          ...
          <div className="flex-1 min-w-0 flex gap-[1vw] items-stretch overflow-hidden">
            {carnesHistory.length === 0 ? (
              ...
              carnesHistory.slice(0, 7).map(item => (
                <div
                  key={item.id}
                ...
                >
                  ...
                  <span className={`text-[2.2vw] font-black leading-none ${
                    item.priority ? 'text-blue-300' : 'text-red-300'
                  }`}>
                    {item.code}
                  </span>
